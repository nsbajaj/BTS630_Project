...
| Number | Task Description | Status |
| --- | --- | --- |
| 1 | Re-Code methods for managing the categories.<br /> - Categories will be only related to cell phones such as Manufacturers, OS, Storage, etc. | Incomplete Categories are handled  but  we have not  made the  categorie  system  reflect this, to do this we need a  full audit of database,  in terms of structure, the database is ready |
| 2 | Re-Code methods for managing prodcuts and inventories.<br /> - Product Page will be reduced to show cell phones only, considering the new changes. | Incomplete, Inventory Page has been created and   methods  related to aproval have been removed, the controller also lacks comunication to database back end |
| 3 | Re-Code methods that handles attributes while adding/viewing the product.<br /> - User will be able to see attributes only related to cell phones such as screen size, memory, colours, etc. | Incomplete  the categories and  atttribbutes have been reworked but  to  get to this step we need a  full recode of  the database in terms of making sure we wipe any and all references to  multi database |
| 4 | Recreate a product page which will show the information about the product (cell phone) in details.<br /> - This page will display all the attributes of the product, images, price, etc. | Incomplete The product page currently  shows and handle the product price quantity and  details but it lacks some basic functionality that remains to be added- image cycling and some information  |
...
| 8 | Admin will be able to see how many orders are left to fulfill (yet to ship).<br /> - Once the customer completes the order, it will show on Admin dashbaord, which needs to be shipped or has not been shipped yet. | Incomplete, Templates and fucntionalitiesd are in place we have database functionality and some basic functionality to show, however we have some basic issues  with security andathentication about how to proceed on this |
| 9 | Admin will be able to update the status of the order.<br /> - If the order has been shipped then the admin can update the status of the order to "Shipped", and customer can see that on their end as well. | Incomplete, The template is there the functionality is  finished the rule about if orders are in place allows user to discontinue the product. the current idea is as long as all orders are fullfilled we can discontinue the product- if 0 stock this is not an issue however. |
| 10 | Update user roles and authentication according the changes made to the system.<br /> - Remove "Seller" aspect/account(s) from previous System | Completed as plannedup to specs |
