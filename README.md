...
| Number | Task Description | Status |
| --- | --- | --- |
| 1 | Re-Code methods for managing the categories.<br /> - Categories will be only related to cell phones such as Manufacturers, OS, Storage, etc. | Categories are handled but we have not made the categories system reflecting the new aspect totally, to do this we need a full audit of database. In terms of structure, the database is ready. |
| 2 | Re-Code methods for managing prodcuts and inventories.<br /> - Product Page will be reduced to show cell phones only, considering the new changes. | Inventory Page has been created and methods related to approval has been removed, the controller lacks communication with database (back end). |
| 3 | Re-Code methods that handles attributes while adding/viewing the product.<br /> - User will be able to see attributes only related to cell phones such as screen size, memory, colours, etc. | The categories and attributes have been reworked but  to get to this step we need a full recode of the database in terms of making sure we wipe all references to  multi database. |
| 4 | Recreate a product page which will show the information about the product (cell phone) in details.<br /> - This page will display all the attributes of the product, images, price, etc. | The product page currently shows and handles the product price, quantity and details but it lacks some basic functionality that remains to be added - image cycling and some information. |
| 5 | User will be able to add/remove product from shopping cart.<br /> - User can add a product from product page by clicking on add to cart, and also remove any product from the shopping cart by clicking remove. | Completed as planned.  |
| 6 | User will be able to place the order.<br /> - Once the user finalizes the shopping cart and places the order, it will generate an order to fulfill for admin. (For payment, we will be adding Paypal in next iteration) | Completed as planned. |
...
| Number | Task Description | Status | 
| --- | --- | --- |
| 1 | Admin will be able to add a product.<br /> - Once the admin clicks on "Create a product", the admin will be able to enter details and attributes about about the product. and once it clicks add a product it will add it to the inventory |   |
| 2 | A user dashboard page will be created, which would have 2 links to navigate to: i) Profile Page ii) Placed Orders. And if the user is an admin, then there will be 2 additional links will be visible: iii) Products/Inventory Page iv) Orders to Ship<br /> - If the user clicks on any of these links, it will navigate/re-direct to that particular page. |   |
| 3 | A Profile page will be created for all users.<br /> - The page will show all the information about the user, along with the ability to edit/change certain information.  |   |
| 4 | An Orders to ship page will be created for admins.<br /> - Admins will be able to keep track of how many orders are left to be shipped and  view or update the status of any order  |   |
| 5 | An Orders page will be created for all users.<br /> - This page will list all the orders that the user has placed, along with the status of each order, whether "Placed" ,"Shipped" or "Complete" , "Cancelled" |   |
| 6 | For payment purpose, Paypal functionality will be added.<br /> - User will be able to pay for their order, once they finalizes the shopping cart.<br />(Need to discuss, whether should we do this or not!) |   |
